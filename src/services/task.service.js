...
const createTaskHandler = async (data) => {
    const { description, prefix, startDate, endDate, status, assignedToType, assignedToId, createdBy } = data;
    ...
    try {
        ...
        const task = await db.task.create({
            data: {
              ...
              assignedToId,
              createdBy,
              taskCodeId: createdTaskCode.id, // Use taskCodeId only here
            },
            include: {
              assignedToUser: true,
              ...
              createdBy: true,
              taskCode: true, // Include in the query result if needed
            },
          });
        ...
        throw new ApiError(httpStatus.INTERNAL_SERVER_ERROR, 'Task creation failed');
    }
};
