...
CREATE TABLE `tasks` (
    ...
    `taskCodeId` VARCHAR(191) NOT NULL,
...
CREATE TABLE `task_codes` (
    ...
    UNIQUE INDEX `task_codes_code_key`(`code`),
    PRIMARY KEY (`id`)
) DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
...
CREATE TABLE `system_codes` (
    `id` VARCHAR(191) NOT NULL,
    `module` ENUM('BATCH', 'PLAN', 'USER_SIGNUP', 'USER', 'ACADEMY_PROGRAM', 'QUIZ', 'QUIZ_QUESTION') NOT NULL,
...
CREATE TABLE `academy_programs` (
    ...
    `duration` ENUM('MONTHLY', 'SEASONAL') NOT NULL,
...
CREATE TABLE `page_components` (
    `id` VARCHAR(191) NOT NULL,
    `type` ENUM('PAGE_HEADER', 'LEVEL_CARD', 'CHESS_BATCH', 'CONTACT_SECTION', 'TEXT_BLOCK', 'IMAGE_BLOCK', 'HERO_SECTION', 'PROGRAM_LIST', 'TESTIMONIAL', 'COACH_PROFILE', 'NEWS_SECTION', 'ACHIEVEMENT_SHOWCASE', 'SEPARATOR', 'SECTION_HEADER') NOT NULL,
    `props` JSON NOT NULL,
    `order` INTEGER NOT NULL,
...
-- AddForeignKey
ALTER TABLE `tasks` ADD CONSTRAINT `tasks_taskCodeId_fkey` FOREIGN KEY (`taskCodeId`) REFERENCES `task_codes`(`id`) ON DELETE RESTRICT ON UPDATE CASCADE;
